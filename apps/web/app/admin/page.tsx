            ...
            {sortedNews.map(item => (
              ...
              >
                ...
                <div className="flex items-center gap-1.5">
                  <button onClick={() => handleEdit(item)} className="p-2 text-gray-400 hover:text-unc hover:bg-unc/5 rounded-lg transition-colors" title="Edit">
                ...
                {people.map(p => (
                  <label
                    ...
                    className={`flex items-center gap-3 p-3 rounded-lg cursor-pointer transition-all hover:bg-white border border-transparent hover:border-gray-200 ${
                      selectedPeopleIds.includes(p.id) ? 'bg-white border-unc/30 shadow-sm ring-1 ring-unc/10' : ''
                    }`}
                  >
                    <div
                      className={`w-5 h-5 rounded flex items-center justify-center transition-colors ${
                        selectedPeopleIds.includes(p.id) ? 'bg-unc text-white' : 'bg-gray-200 text-transparent'
                      }`}
                    >
          ...
          <div className="card overflow-hidden">
            <div className="p-8">
              <div className="flex items-center gap-2 mb-4">
                <span className="px-3 py-1 rounded-full bg-unc/10 text-unc text-xs font-bold uppercase tracking-wider">Update</span>
